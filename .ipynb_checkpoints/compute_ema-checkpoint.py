...
weight_files = [weights_folder / f for f in weights_folder.iterdir() if ".pt" in f.name]
weight_files = sorted(weight_files, key=lambda x: int(''.join(filter(str.isdigit, x.name))))
weight_files = weight_files[:100]
